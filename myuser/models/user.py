...
from common.utils import date_expired_default, get_signer
...
class User(AbstractUser):
    ...
    groups = models.ManyToManyField('myuser.UserGroup', related_name='users', blank=True, verbose_name=_("User group"))
    role = models.CharField(choices=ROLE_CHOICES, default=ROLE_USER, max_length=10, blank=True, verbose_name=_("Role"))
    ...
    otp_level = models.SmallIntegerField(choices=OTP_LEVEL_CHOICES, default=0, verbose_name=_("MFA"))
    ...
    date_expired = models.DateTimeField(default=date_expired_default, blank=True, null=True, db_index=True,
                                        verbose_name=_("Date expired"))
    created_by = models.CharField(max_length=30, default='', verbose_name=_("Created by"))
    source = models.CharField(max_length=30, default=SOURCE_LOCAL, choices=SOURCE_CHOICES, verbose_name=_("Source"))
    ...
    def get_absolute_url(self):
        return reverse("myuser:user-detail", args=(self.id,))
    ...
    def private_token(self):
        return self.get_or_create_private_token()

    def get_or_create_private_token(self):
    ...
    def get_or_create_access_key(self):
        from . import AccessKey
        access_key, status = AccessKey.objects.get_or_create(user=self)
        return access_key, status

    def refresh_access_key(self):
        from . import AccessKey
        AccessKey.objects.filter(user=self).delete()
        return AccessKey.objects.create(user=self)
    ...
    def check_public_key(self, public_key):
        if self._public_key == public_key:
            return True
        else:
            return False
    ...
    def create_app_user(cls, name, comment):
        app = cls.objects.create(
            ...
            is_first_login=False, create_by='System'
        )
        access_key, status = app.get_or_create_access_key()
